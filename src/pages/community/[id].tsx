...
const Community = () => {
	...
	return (
		<Container fluid>
			<Image
				src={COMMUNITIES?.[id].cover}
				alt={COMMUNITIES?.[id].name}
				width="100%"
				height="30vh"
				fit="cover"
				radius="md"
			...
			<Grid>
				...
				<Grid.Col span={4}>
					<Card withBorder>
						<Title mb={5}>{COMMUNITIES?.[id].name || ""}</Title>
						<Title mb={10} order={3} size="sm" color="dimmed">
							About Community
						</Title>
						<Text>{COMMUNITIES[id].description}</Text>
						<Text>
							<Group>
								<Text fw={800}>Created by:</Text>
								<Link href={`/profile/`} passHref>
									<Text component="a">
										{COMMUNITIES[id].creator}
									</Text>
								</Link>
							</Group>
						</Text>
						...
						<Avatar.Group spacing="xs">
							{COMMUNITIES[id]?.members?.map((member) => (
								<Tooltip key={member.id} label={member.name}>
									<Avatar
										src={member.avatar}
										alt={member.name}
